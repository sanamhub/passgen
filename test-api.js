...
async function testApi() {
  ...
  try {
    ...
    if (!defaultResponse.ok) {
      throw new Error(
        `Error: ${defaultResponse.status} ${defaultResponse.statusText}`
      );
    }
    ...
    const easyToSayResponse = await fetch(
      `${API_URL}/api/generate?characterMode=easyToSay`
    );
    ...
    const customResponse = await fetch(
      `${API_URL}/api/generate?uppercase=false&symbols=false`
    );
